...
const objLat = (obj) => {
    ...
    let firstSlice = obj.firstName.slice(1);
    let lastSlice = obj.lastName.slice(1);
    return `my name is ${obj.firstName.charAt(0).toUpperCase()}${firstSlice} ${obj.lastName.charAt(0).toUpperCase()}${lastSlice} I am ${obj.age} YO, and I love ${obj.hobby}.`
};
// -------------------------------------------------------------------------------------------------------
...
const classesAvg = (data) => {
    ...
    for(let i=0 ; i < data.grades.length ; i++){
        for(let classCount=0 ; classCount < data.grades[i].classes.length ; classCount++){
            let sum = 0;
            for(let studCount=0; studCount < data.grades[i].classes[classCount].classScores.length ; studCount++){
                sum += data.grades[i].classes[classCount].classScores[studCount];
            }
            data.grades[i].classes[classCount].avg = Math.floor(sum / data.grades[i].classes[classCount].classScores.length)
        }
    }
    return data ;
};
